...
export const authToken_Interceptor: HttpInterceptorFn = (req, next) => {
  ...
  return next(req).pipe(
    //Если ошибка перехватываем и делаем запрос на обновление токена, затем переключаемся(switchMap) на другой стрим(создаем новый Observable) с новым токеном и делаем повторный запрос next(реквест)
    catchError((erorr) => {
      if (erorr.status === 403) {
...
/*
Итог:
  подписчик subscriber видит либо успех с новыми данными, 
  либо ошибку, если обновление токена и повторный запрос тоже не удались.
