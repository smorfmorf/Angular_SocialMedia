...
//* (Если получим ответ что токен не валиден,тогда мы запросим новый токен) 
  ...
  */
  ...
  return next(req).pipe(
    //Если ошибка перехватываем и делаем запрос на обновление токена, затем переключаемся(switchMap) на другой стрим с новым токеном и затем делаем повторный запрос next(req);
    catchError((erorr) => {
      if (erorr.status === 403) {
